...
# config/deploy.rb
set :application, 'chromatic'
set :repo_url, 'git@github.com:zrandles/chromatic.git'
set :branch, 'main'
...
namespace :deploy do
  ...
  task :run_tests do
    run_locally do
      ...
      # Run RSpec tests
      execute :bundle, :exec, :rspec, '--format', 'documentation'
  ...
  task :restart_puma_sudo do
    on roles(:app) do
      execute :sudo, :systemctl, :restart, 'chromatic.service'
    end
  end
